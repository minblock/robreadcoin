...
const char * const BITCOIN_CONF_FILENAME = "robreadcoin.conf";
...
#else
    const char* pszModule = "robreadcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "RoBreadCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/RoBreadCoin";
#else
    // Unix
    return pathRet / ".robreadcoin";
#endif
#endif
}
